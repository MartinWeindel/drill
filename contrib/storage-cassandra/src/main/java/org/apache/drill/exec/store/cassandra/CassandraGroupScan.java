...
public class CassandraGroupScan extends AbstractGroupScan implements DrillCassandraConstants {
    ...
    private void init() {
        try {
            logger.info(String.format("Getting cassandra session from host %s, port: %s, user: %s.",
                    storagePluginConfig.getHosts(), storagePluginConfig.getPort(), "" + storagePluginConfig.getUser()));

            cluster = CassandraConnectionManager.getCluster(storagePluginConfig.getHosts(),
                    storagePluginConfig.getPort(), storagePluginConfig.getUser(), storagePluginConfig.getPassword());
    ...
    private CassandraSubScanSpec hostToSubScanSpec(Host host, List<String> contactPoints) {
        ...
        return  new CassandraSubScanSpec()
                ...
                .setUser(storagePluginConfig.getUser())
                .setPassword(storagePluginConfig.getPassword())
